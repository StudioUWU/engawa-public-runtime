...
set(PATCHES
    ...
    # Static data belongs inside EngawaRuntimeUtils but is not part of its export ABI.
    ${CMAKE_CURRENT_LIST_DIR}/patches/0005-Do-not-export-static-data-entry-point.patch
)
...
if (ENABLE_TOOLS)
    ...
    # Config fixup runs before the vcpkg tool relocation above.  Rewrite the
    # exported locations for tools that ICU originally installs in sbin so
    # find_package(ICU CONFIG) does not retain dangling imported targets.  An
    # imported executable occurs once as its location and once in CMake's
    # generated existence check; require both references before rewriting.
    foreach(config_file IN ITEMS
        ${CURRENT_PACKAGES_DIR}/share/icu/ICUTargets-debug.cmake
        ${CURRENT_PACKAGES_DIR}/share/icu/ICUTargets-release.cmake)
        if (NOT EXISTS "${config_file}")
            message(FATAL_ERROR
                "ICU exported-target config is missing after fixup: ${config_file}")
        endif ()
        file(READ "${config_file}" config_contents)
        foreach(tool IN LISTS STOOLS)
            set(old_location
                "\${_IMPORT_PREFIX}/sbin/${tool}${TOOL_EXTENSION}")
            set(new_location
                "\${_IMPORT_PREFIX}/tools/icu/${tool}${TOOL_EXTENSION}")
            string(LENGTH "${config_contents}" before_length)
            string(REPLACE "${old_location}" "" config_without_old_location
                "${config_contents}")
            string(LENGTH "${config_without_old_location}" after_length)
            string(LENGTH "${old_location}" old_location_length)
            math(EXPR replaced_byte_count "${before_length} - ${after_length}")
            math(EXPR old_location_count
                "${replaced_byte_count} / ${old_location_length}")
            if (NOT old_location_count EQUAL 2)
                message(FATAL_ERROR
                    "Expected exactly two ${old_location} references in "
                    "${config_file}; found ${old_location_count}")
            endif ()
            string(REPLACE "${old_location}" "${new_location}" config_contents
                "${config_contents}")
        endforeach()
        string(FIND "${config_contents}" "\${_IMPORT_PREFIX}/sbin/"
            remaining_sbin_location)
        if (NOT remaining_sbin_location EQUAL -1)
            message(FATAL_ERROR
                "ICU exported-target config retains an sbin location: ${config_file}")
        endif ()
